...
namespace RE::StorageTable {
    ...
    struct alignas(0) CameraCut {
        uint8_t data[1800]; // 0x0
    };
    static_assert(sizeof(CameraCut) == 0x708);
    ...
    struct CameraTransformData {
        uint8_t data[0x14]; // most likely 5 floats
    };
    static_assert(sizeof(CameraTransformData) == 0x14); // confirmed
    ...
    namespace Camera {
        struct alignas(8) Storage {
            ...
            ObserverDirectStorageData<Storage, void *> *pObserverStorageData; // 0x38 struct HostOnlyMemory<StorageTable::CameraBlocks>
            void *buffer1; // byte buffer
            void *buffer2; // byte buffer
        };
